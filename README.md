...
단순 배포를 넘어 서비스 안정성과 확장성을 보장하는 엔터프라이즈급 인프라를 구축했습니다.
...
* CI/CD 및 Serverless 배포: Vercel을 이용한 배포 파이프라인 구축 및 환경 변수 자동화 관리(Dev/Prod 분리 운영)
...
* RESTful API 구현: Spring Boot를 사용하여 회원가입, 주행 기록 저장/조회 등 핵심 비즈니스 로직 구현
...
* 모바일 앱(App) 구현: React Native를 사용하여 Android 네이티브 앱 클라이언트 전담 개발
